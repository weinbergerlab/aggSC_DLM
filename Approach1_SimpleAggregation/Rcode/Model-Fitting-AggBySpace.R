...
# (Simple aggregation across *space*)
...
####################### Select #######################
agglvl <- "National" # Region or National
...
call.jags.mod <- function(ds, rand.eff) {
 
 #-----*-----*-----*-----*-----*-----*-----*-----#
 # Set up                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           et up for each state
 #-----*-----*-----*-----*-----*-----*-----*-----#
 
 # source('~/Approach1_SimpleAggregation/Rcode/Aim3a_functions.R', local = TRUE)
 source('~/Approach1_SimpleAggregation/Rcode/synthetic_control_functions.R', local = TRUE)
 
 # Log-transform all variables, after adding 0.5 to all controls.
 # NOTE: Time-aggregated data are already in a log scale, so we don't have to do this.
 ds[,4:ncol(ds)] <- ds[,4:ncol(ds)] + 0.5 # First 3 columns are age_group, date, and the outcome
 ds[,4:ncol(ds)] <- log(ds[,4:ncol(ds)])
 
 # Create monthly dummy variables and an indicator for the 2009 flu pandemic
 months <- month(ds[, date_name])
 month.mat <- dummies::dummy(months)
 month.mat <- month.mat[,-12]
 if (intervention_date > as.Date('2009-09-01')) { # This indicator variable controls for the 2009 flu epidemic
  pandemic <- ifelse(ds[, date_name] == '2009-08-01', 1, ifelse(ds[, date_name] == '2009-09-01', 1, 0))
 }
 month.mat <- cbind(rep(1, nrow(month.mat)), month.mat, pandemic) # Add an intercept
 colnames(month.mat) <- c("Intercept","s1","s2","s3","s4","s5","s6","s7","s8","s9","s10","s11","pandemic")
 month.mat.pre <- month.mat[ds[, date_name] < post_period[1], ]
 
 # Eliminates effects from 2008 coding change in Brazil
 covars <- ds[, 4:ncol(ds)]
 time_points <- unique(ds[, date_name])
 month_i <- as.factor(as.numeric(format(time_points, '%m')))
 spline <- setNames(as.data.frame(bs(1:nrow(covars), knots = 5, degree = 3)), 
                    c('bs1', 'bs2', 'bs3', 'bs4'))
 ...
 # Scale control time series
 x.scale <- as.data.frame(lapply(covars[, apply(covars, 2, var) != 0, drop = FALSE], scale), 
                          check.names = FALSE)
 
 # Reformat the dataset
 y <- ds[,'J12_18']
 y.pre <- y[ds$date < post_period[1]]
 ds2 <- cbind.data.frame(y, x.scale)
 names(ds2) <- c('J12_18', names(x.scale))
 ds2.pre <- ds2[ds$date < post_period[1],]
 ds3 <- ds2
 ds3$J12_18[ds$date >= post_period[1]] <- NA
 ...
 # Select the model
 if (rand.eff=="AR1") {
  mod.txt <- source('~/Approach1_SimpleAggregation/Rcode/Horseshoe/Non_spatial-Non_lagged-AR1_KS.R') 
 } else if (rand.eff=="IID") {
  mod.txt <- source('~/Approach1_SimpleAggregation/Rcode/Horseshoe/Non_spatial-Non_lagged-independent_KS2.R') 
 }
...
clusterExport(cl, c('call.jags.mod','post_period','date_name','intervention_date'), environment())
