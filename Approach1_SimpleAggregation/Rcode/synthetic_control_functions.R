...
doCausalImpact <- function(zoo_data, 
                           intervention_date, 
                           ri.select=TRUE,
                           time_points,
  ...
  if(trend){
    ...
    bsts_model.pois  <- poissonBvs(y=y.pre , 
                                   X=x.pre, 
                                   offset=offset.t.pre, 
                                   BVS=var.select.on, 
                                   model = list(
  ...
  if(ri.select){
    disp<-bsts_model.pois$samplesP$thetaBeta[-c(1:burnN),1] ##note theta beta is signed--bimodal dist---take abs value
  ...
  reg.mean.fitted<- exp((x.fit.pre %*% t(beta.mat)) + t(rand.eff) )
...
rrPredQuantiles <- function(impact, denom_data = NULL,  eval_period, post_period) {
  ...
  # unbias_rr_q<- quantile(unbias_rr, probs = c(0.025, 0.5, 0.975), na.rm=TRUE)
  ...
  quantiles <- list(#unbias_rr_q=unbias_rr_q, 
...
plotPredAgg <- function(ann_pred_quantiles,  time_points, post_period, #ylim, 
  ...
  #   labs(x = 'Year', y = 'Number of pneumonia deaths') + 
